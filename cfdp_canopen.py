...
class SimpleCfdpUser(CfdpUserBase):
    ...
    def _complete_transaction(self, transaction_id: TransactionId):
        ...
        with self._lock:
            # Keep the just-set event so late callers still see it
            self._finished_events = {
                tid: ev for tid, ev in self._finished_events.items()
                if not ev.is_set() or tid == transaction_id
            }
    ...
    def transaction_finished_indication(self, params: TransactionFinishedParams):
        ...
        self._complete_transaction(params.transaction_id)
